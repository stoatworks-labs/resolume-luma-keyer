...
void testChannelOrder()
{
	...
	lumakey::keyTile( bgraIn, 4, lumakey::Layout::BGRA8, bgraOut, 4, lumakey::Layout::BGRA8, 1, 1, kState );
	lumakey::keyTile( rgbaIn, 4, lumakey::Layout::RGBA8, rgbaOut, 4, lumakey::Layout::RGBA8, 1, 1, kState );
	...
	check( rgbaOut[ 0 ] == lumakey::quantise8( expect[ 0 ] ), "red matches the shared core" );
	check( rgbaOut[ 3 ] == lumakey::quantise8( expect[ 3 ] ), "alpha matches the shared core" );
...
void testStridePadding()
{
	...
	lumakey::keyTile( src.data(), stride, lumakey::Layout::RGBA8,
					  dst.data(), stride, lumakey::Layout::RGBA8, w, h, kState );
	...
	for( int y = 0; y < h; ++y )
	{
		...
		for( int x = 0; x < w; ++x )
		{
			...
			if( p[ 3 ] != lumakey::quantise8( expect[ 3 ] ) )
				rowsRight = false;
		}
	}
...
void testHalfFloatRoundTrip()
{
	...
	bool ok = true;
	for( float v : values )
		if( std::fabs( lumakey::halfToFloat( lumakey::floatToHalf( v ) ) - v ) > 0.001f )
			ok = false;
	...
	const float v = 0.6f;
	uint16_t src[ 4 ] = { lumakey::floatToHalf( v ), lumakey::floatToHalf( v ),
						  lumakey::floatToHalf( v ), lumakey::floatToHalf( 1.0f ) };
	uint16_t dst[ 4 ] = {};
	lumakey::keyTile( reinterpret_cast<uint8_t*>( src ), 8, lumakey::Layout::RGBAh,
					  reinterpret_cast<uint8_t*>( dst ), 8, lumakey::Layout::RGBAh, 1, 1, kState );

	float expect[ 4 ];
	reference( v, v, v, 1.0f, expect );
	check( std::fabs( lumakey::halfToFloat( dst[ 3 ] ) - expect[ 3 ] ) < 0.002f,
...
void testFloatAndMixedLayouts()
{
	...
	float dstF[ 4 ] = {};
	lumakey::keyTile( reinterpret_cast<uint8_t*>( src ), 16, lumakey::Layout::RGBAf,
					  reinterpret_cast<uint8_t*>( dstF ), 16, lumakey::Layout::RGBAf, 1, 1, kState );
	...
	uint8_t dst8[ 4 ] = {};
	lumakey::keyTile( reinterpret_cast<uint8_t*>( src ), 16, lumakey::Layout::RGBAf,
					  dst8, 4, lumakey::Layout::BGRA8, 1, 1, kState );
	check( dst8[ 3 ] == lumakey::quantise8( expect[ 3 ] ), "float source into 8-bit destination" );
}
...
void testKeyBehaviour()
{
	...
	auto alphaFor = [ ]( uint8_t luma ) {
		...
		uint8_t dst[ 4 ] = {};
		lumakey::keyTile( src, 4, lumakey::Layout::RGBA8, dst, 4, lumakey::Layout::RGBA8, 1, 1, kState );
		return dst[ 3 ];
	};
	...
	uint8_t dst[ 4 ] = {};
	lumakey::keyTile( src, 4, lumakey::Layout::RGBA8, dst, 4, lumakey::Layout::RGBA8, 1, 1, inverted );
